...
const CONFIG = {
  ...
  GOOGLE_MAPS_KEY: 'YOUR_GOOGLE_MAPS_KEY_HERE', // Replace with actual key
...
const Auth = {
  ...
  // FIXED: Added missing requireAuth method to prevent TypeErrors in dashboards
  requireAuth(allowedRoles = []) {
    ...
    if (allowedRoles.length && !allowedRoles.includes(user.role)) {
      window.location.href = '/frontend/auth/unauthorized.html';
      return false;
    }
    return true;
  },
...
const UI = {
  ...
  formatNGN(amount) {
    return `₦${parseFloat(amount||0).toLocaleString('en-NG',{minimumFractionDigits:0})}`;
  },
...
// Inject global CSS keyframes for animations used in config and UI
